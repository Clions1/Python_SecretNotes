...
def decryptNotes():
    ...
    if len(message_encrypted)==0 or len(master_secret)==0:
        message_encrypted.showinfo(title="Error!",messagebox="Please enter all info")
    else:
            try:
...
title_info_label=Label(text="Enter your title",font=FONT,bg="black",fg="white")
title_info_label.pack()

title_entry=Entry(width=30)
title_entry.pack()

title_info_label=Label(text="Enter your note",font=FONT,bg="black",fg="white")
title_info_label.pack()

input_text=Text(width=50,height=25)
input_text.pack()

master_secret_label = Label(text="Enter master key",font=FONT,bg="black",fg="white")
master_secret_label.pack()

master_secret_input=Entry()
master_secret_input.pack()

save_button=Button(text="Save & Encrypt",command=save_and_encrypt)
save_button.pack()

decrypt_button=Button(text="Decrypt",command=decryptNotes)
decrypt_button.pack()
